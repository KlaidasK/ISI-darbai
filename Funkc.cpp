...
void isrinkimas(vector<studentas> grupe, vector<studentas>& Nel, vector<studentas>& Kiet) {
	auto start3 = std::chrono::high_resolution_clock::now(); auto st3 = start3; //Pradedamas dalijimas į dvi grupes
	studentas nelaim;
	studentas kiet;
	for (int i = 0; i < grupe.size(); i++) {										//Rusiuojama į dvi skirtingas grupes pagal vidurki
		grupe[i].vid = vidurk(grupe[i]);
		if (grupe[i].vid < 5.0) {			//Nelaimeliai išskirstomi
			nelaim = grupe[i];
			Nel.push_back(nelaim);
		}
		else if (grupe[i].vid >= 5.0) {		//Kietekai išskirstomi
			kiet = grupe[i];
			Kiet.push_back(kiet);
		}
	}
