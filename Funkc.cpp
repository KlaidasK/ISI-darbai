...
void generate(studentas& temp, int nd_kiekis) { // Atsitiktinio generavimo funkcija
	...
	const int size = 10;
	std::uniform_int_distribution<int>dist(0, size-1); // Vardam ir pavardėm
	if (size - 1 >= size) throw("Vardu generavimo klaida!");
	string Vard[size]{ "Tomas", "Jonas", "Aleksas", "Benas", "Juozas", "Jurgis", "Domas", "Naglis", "Lukas", "Martynas" };
	string Pav[size]{ "Abligaitis", "Baravicius", "Kristinis", "Mencys", "Plienys", "Plieskis", "Skeitaitis", "Skepenis", "Vunderis", "Zabalskis" };
