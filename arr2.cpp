...
int main()
{
    ...
    else { cout << "Ivyko klaida!" << endl; return 0; } // Kuriamas tuščias masyvas, jei įvyksta klaida
...
void pild(studentas& temp, int& nd_kiekis) {
    ...
    {
        do {
            if (cin >> p || p < 0 || p>10) break;
            cin.clear();
            cin.ignore(numeric_limits<std::streamsize>::max(), '\n');
        } while (true); // Tikrina ar pažymys yra skaičius ir ar jis tarp 0 ir 10
...
float median(studentas temp, int nd_kiekis)
{
    ...
    arrkopija[newsize-1] = temp.egz; // Prie masyvo pridedamas ir egzamino pažymys
