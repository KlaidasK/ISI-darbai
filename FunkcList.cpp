...
void skaitymas(std::string read_vardas, list<studentas>& grupe, int& max) {
	...
	for (int i = 0; i < sk; i = i + stul) {
		temp.vardas = *(next(it, i));
		temp.pavarde = *(next(it, i + 1));
		for (int j = i + 2; j < i + stul - 1; j++) temp.paz.push_back(stoi(*(next(it, j))));
		temp.egz = stoi(*(next(it, i + stul - 1)));
...
void isrinkimas(list<studentas> grupe, list<studentas> &Nel, list<studentas> &Kiet) {
